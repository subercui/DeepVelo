...
if DEEPVELO == 'ShowTarget':
    ...
    now = time()
    os.system(
        f'python train.py -c config_figure2.json --ng {n_genes} --bs {batch_size} --ot {DEEPVELO_FILE} --dd ./data/scveloDG.npz')
...
# %% plot panel a
if data == 'DG':
    scv.pl.velocity_embedding_stream(adata, basis='umap', color='clusters', legend_fontsize=9,
                                     dpi=300, save=f'figure2/velo_emb_stream{SURFIX}.png')
    # scv.pl.velocity_embedding(adata, basis='umap', arrow_length=1.2, arrow_size=1.2, dpi=150)
    scv.pl.velocity_embedding_grid(adata, basis='umap', arrow_length=3, arrow_size=2,
                                   dpi=300, save=f'figure2/velo_emb_grid{SURFIX}.png')

# %% panel b
scv.pl.velocity(adata, var_names=['Tmsb10', 'Ppp3ca', 'Dlg2'], basis='umap',
                dpi=300, save=f'figure2/phase_velo_exp{SURFIX}.png')
scatter(adata, var_names=['Tmsb10'], basis='umap',
        add_quiver=True, dpi=300, save=f'figure2/phase{SURFIX}1.png',
        legend_loc_lines="none")
scatter(adata, var_names=['Fam155a'], basis='umap',
        add_quiver=True, dpi=300, save=f'figure2/phase{SURFIX}2.png',
        legend_loc_lines="none")
# %% more plots
...
if data == 'EP':
    ...
    scatter(adata, var_names=['Sntg1', 'Sbspon'], basis='umap',
            add_quiver=True, dpi=300, save=f'figure2/phase{SURFIX}.png')
elif data.startswith('E9'):
    scv.pl.velocity(adata, var_names=['Mybl1', 'Rragb'], basis='pca',
                    dpi=300, save=f'figure2/phase_velo_exp{SURFIX}.png')
    scatter(adata, var_names=['Mybl1'], basis='pca', add_quiver=True, dpi=300, save=f'figure2/phase{SURFIX}.png')
    scatter(adata, var_names=['Rragb'], basis='pca', add_quiver=True, dpi=300, save=f'figure2/phase{SURFIX}.png')
...
scv.pl.scatter(
    ...
    size=80,
    dpi=300,
    save=f'figure2/latent_time{SURFIX}.png'
)
