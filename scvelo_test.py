...
if DEEPVELO == 'ShowTarget':
    ...
    # os.system(f'python train.py -c config_test_gin.json --ng {n_genes} --bs {batch_size} --ot {DEEPVELO_FILE} --dd ./data/scveloDG.npz')
    os.system(
        f'python train.py -c config_test_gcn.json --ng {n_genes} --bs {batch_size} --ot {DEEPVELO_FILE} --dd ./data/scveloDG.npz')
...
if data == 'DG':
    scv.pl.velocity_embedding_stream(adata, basis='umap', color='clusters',
                                     dpi=300, save=f'velo_emb_stream{SURFIX}.png')
    # scv.pl.velocity_embedding(adata, basis='umap', arrow_length=1.2, arrow_size=1.2, dpi=150)
    scv.pl.velocity_embedding_grid(adata, basis='umap', arrow_length=1.2, arrow_size=1.2,
                                   dpi=300, save=f'velo_emb_grid{SURFIX}.png')
elif data == 'EP':
    ...
    scv.pl.velocity_embedding_stream(adata, basis='pca', color='time', dpi=300,
                                     title=f'{data}_pca', save=f'{data}velo_stream{SURFIX}[pca].png')
elif data == 'velocyto_hg':
    scv.pl.velocity_embedding_stream(adata, basis='pca', color='Clusters', dpi=300,
                                     title=f'{data}_pca', save=f'{data}velo_stream{SURFIX}[pca].png')
elif data == 'velocyto_dg':
    scv.pl.velocity_embedding_stream(adata, basis='tsne', color='Clusters', dpi=300,
                                     save=f'{data}velo_stream{SURFIX}[tsne].png')
# %% more plots
