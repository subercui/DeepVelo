...
def filter_from_file(vlm, barcode_file):
    ...
    # {'GGCAGTCCACATCCCT': 10,}
    barcode_mapping = {row['barcode'][-18:-2]:row['cluster'] for _, row in barcode_df.iterrows()}
    lineage_mask = np.zeros(len(vlm.ca['CellID']), dtype=bool)
    cell_type = np.zeros(len(vlm.ca['CellID']), dtype=int)
    for i in range(len(lineage_mask)):
        cell_code = vlm.ca['CellID'][i][-17:-1]
        if cell_code in barcode_mapping:
            lineage_mask[i] = True
            cell_type[i] = int(barcode_mapping[cell_code])
    vlm.ca['cell_type'] = cell_type
...
if DATA == "9-11":
    ...
    kw = dict(num=None)
    DEEPVELO_FILE = 'velo_mat_E9F1.npz'
# down sample
if CLUSTER_CODE:
...
vlm.plot_grid_arrows(scatter_kwargs_dict={"alpha":0.6, "lw":0.7, "edgecolor":"0.4", "s":70, "rasterized":True, "c":color_set, "cmap":mycmp},
...
plt.figure(None,(12,12),dpi=300)
vlm.plot_grid_arrows(scatter_kwargs_dict={"alpha":0.7, "lw":0.7, "edgecolor":"0.4", "s":70, "rasterized":True, "c":color_set, "cmap":mycmp},
                     min_mass=2.9, angles='xy', scale_units='xy',
